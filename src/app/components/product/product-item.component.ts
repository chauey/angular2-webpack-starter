...
@Component({
  ...
  pipes: [],
  providers: [],
  styles: [require('bootstrap/dist/css/bootstrap.min.css'), require('./product-item.component.css')],
...
export class ProductItemComponent implements OnInit {
  ...
  ngOnInit() {
    console.log('ngOnInit product-item component');

    this.id = this._routeParams.get('id') === 'new' ? null : +this._routeParams.get('id');
    console.log('ngOnInit product-item component id is: ' + this.id);

    if (this.id === null || this.id === undefined) {
      this.model = {};
    } else {
      this.model = this._dataService.getProduct(this.id);
    }
  ...
  onSubmit() {
    ...
    if (true) { // this.inventoryItemForm.form.isValid) {
      ...
      if (this.id !== null) {
        ...
        // add to data repo
        this._dataService.productList.push(this.model);
      }
